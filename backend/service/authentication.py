...
def confirm_registartion(token):
    ...
    if not c:
        return jsonify({'error': "ce token n'existe pas"}), 400
    if c.status == 'enabled':
        return jsonify({'error': 'ce compte est déjà activé'}), 400
    ...
    if compte:
        if compte.password == password and compte.status == 'enabled':
            ...
            if compte.password != password:
                ...
                return jsonify({'error': 'this account is disabled, please confirm registration to sign in'}), 400
    else:
...
def check_code(email, code):
    ...
    if not db_code:
        return jsonify({'error': 'email or code does not exist'}), 400
    else:
        if db_code.is_expired():
            return jsonify({'error': 'the code is expired'}), 400
        else:
            return jsonify({'success': 'code is verified successfully, you can update your password'})
